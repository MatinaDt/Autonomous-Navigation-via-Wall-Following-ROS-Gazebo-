...
class mymobibot_follower():
    ...
    def publish(self):
        ...
        # Reference distances
        d_FR_ref = 0.5
        d_R_ref = np.sqrt(2)*d_FR_ref/2 - L1
        d_FL_ref = 0.8
        ...
        while not rospy.is_shutdown():
            ...
            if (state == 0):
                ...
                print("Wall approaching")
                self.velocity.linear.x = 0.35
                self.velocity.angular.z = 0.0
               

                # Next state - Turn to begin Wall following
                if (self.sonar_F.range < 0.7):
                   state = 1 
                ...
                d_R = self.sonar_R.range 
                d_FR = self.sonar_FR.range
                d_FL = self.sonar_FL.range
                ...
                self.velocity.angular.z = Kp1*(d_FR-d_FR_ref) + u*Kp2*(d_FL-d_FL_ref)
            ...
            if t_save == 0 and t==0:
                output.write(str(t))
                output.write("\t")
                output.write(str(self.sonar_R.range))
                output.write("\t")
                output.write(str(self.sonar_FR.range))
                output.write("\t")
                output.write(str(self.sonar_L.range))
                output.write("\t")
                output.write(str(self.velocity.angular.z))
                output.write("\t")
                output.write(str(self.velocity.linear.x))
                output.write("\t")
                output.write(str(self.imu.orientation.x))
                output.write("\t")
                output.write(str(self.imu.orientation.y))
                output.write("\t")
                output.write(str(self.imu.orientation.z))
                output.write("\n")
                t_save=t
            elif t-t_save >= 0.01:
                output.write(str(t))
